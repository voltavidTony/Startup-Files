...
def plugin_loaded():
	sublime.active_window().run_command("open_startup_files")

# Ensure all specified files are opened and assigned a tab
class OpenStartupFilesCommand(sublime_plugin.WindowCommand):
	def run(self):
...
class SetStartupFilesCommand(sublime_plugin.WindowCommand):
	def run(self):
